...
const AddRecord = () => {
  ...
  const handleAISuggestCategory = async () => {
    ...
    try {
      ...
      if (result.error) {
        // Check if it's a rate limit or AI unavailable situation
        if (result.error.includes('rate limit') || result.error.includes('unavailable')) {
          setCategory(result.category);
          setAlertMessage(`Smart categorization: ${result.category} (AI temporarily unavailable, used keyword matching)`);
      ...
      setAlertMessage('Failed to get category suggestion');
      setAlertType('error');
    } finally {
